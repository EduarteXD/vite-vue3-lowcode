...
export default {
  ...
  preview: () => (
    <div style="text-align:center;">
      ...
      <el-text class="block">文本</el-text>
    </div>
  ),
  render: ({ props, block, styles }) => {
    ...
    return () => (
      <div style={styles}>
        <Image class="block" ref={(el) => registerRef(el, block._vid)} {...props} />
        <div ref={(el) => registerRef(el, block._vid)}>{props.text}</div>
      </div>
    );
  },
  props: {
    ...
    loadingIcon: createEditorInputProp({
      label: '加载时提示的图标名称或图片链接',
    }),
